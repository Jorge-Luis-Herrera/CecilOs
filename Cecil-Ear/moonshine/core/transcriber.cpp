...
Transcriber::Transcriber(const TranscriberOptions &options)
  ...
  if (options.identify_speakers) {
    ...
    if (load_error != 0) {
      this->options.identify_speakers = false;
      delete this->speaker_embedding_model;
      this->speaker_embedding_model = nullptr;
      LOGF("%s", "Warning: Failed to load speaker embedding model from memory (likely dummied out). Speaker identification will be disabled.");
    } else {
